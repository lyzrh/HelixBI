...
4. **`app/graph.py` 保持线性 LangGraph 流程**，只允许通过 `AgentState.skill_block` 这类最小钩子扩展，不做 tool-calling 重构。
5. **指标定义以语义包为唯一来源**：生成提示词必须经 `app/semantic.py` 注入 `semantics/*.yaml`，防止 LLM 即兴发挥导致口径漂移。
...
| 后端 routers/services/模型改动 | [.agents/rules/backend.md](.agents/rules/backend.md) |
...
- Python 3.11+；类型注解尽量齐全；遵循现有 routers/services 分层，不要在 router 里写业务逻辑。
